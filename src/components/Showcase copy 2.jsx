...
//    },[])
   ...
   //   <div onMouseMove={(e) => {moveItems(e.clientX, e.clientY)}} onMouseEnter={(e) => {manageModal(true, index, e.clientX, e.clientY)}} onMouseLeave={(e) => {manageModal(false, index, e.clientX, e.clientY)}} className='relative container '>
       <MouseWrapper className={'relative container bg-slate-500'} cursor={cursor} active={active} index={index} setModal={setModal} modal={modal}>
        <div className='flex gap-4  breaker' >
          <motion.div  className= 'font-custom2 uppercase text-para p-8 z-[9999] pointer-events-none text-neutral-800 !cursor-pointer bg-[#C9FD74] h-10 w-10 fixed top-0 rounded-full flex justify-center items-center' ref={cursor}  variants={scaleAnimation} initial="initial" animate={active ? "enter" : "closed"}>
            View
           </motion.div>
   ...
   <div  class="relative h-[60vh] block lg:hidden  overflow-clip">
      ...
      <svg className="absolute z-40 bottom-0 left-0 size-8 rotate-90" viewBox="0 0 114 114" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M13.9395 114C19.2774 114 24.3941 111.839 28.1518 107.999L108.202 26.1797C111.917 22.3833 114 17.2545 114 11.9096L114 1.35944e-06L114 114L0 114L13.9395 114Z" fill="#222222"></path></svg>
       <motion.div ref={smallScreenImageRef} style={{filter: "blur(0px)",scale:scaleSmallScreen}} class=" relative h-[100%] w-full">
...
function Card({i,img,title,targetScale,range,progress}) {
   ...
   return(   
   <motion.div key={i} ref={targetRef} style={{scale:scaleCard,top:`calc(4% + ${i*35}px)`}} className= ' my-4 mx-auto w-[100%%] rounded-[20px] overflow-hidden noise h-[90vh] sticky top-[0px]'>
