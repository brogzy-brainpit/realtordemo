...
const HorizontalScrollCarousel = () => {
  ...
  const x = useTransform(scrollYProgress, [0, 1], ["1%", "-82.5%"]);

  return (
    <section ref={targetRef} className="relative h-[300vh] bg-eutral-900">
      <div className="sticky top-0 flex h-screen items-center overflow-hidden">
        <motion.div style={{ x }} className="flex gap-4 flex-row">
          {cards.map((card) => {
            return <Card card={card} key={card.id} />;
          })}
        <h2 className="ml-[6vw] md:ml-[10vw] capitalize text-[80px] leading-[88px] md:text-[90px] md:leading-[78px] lg:text-[123px] lg:leading-[118px] font-custom">
        <span className="text-[#C9FD74] capitalize">
          great service 
          </span>
          <br/> 
          <span className="text-[#C9FD74] capitalize pl-6">
          -great result
          </span>
  ...
  <div className="transition-all duration-300 ease-in-out w-full absolute top-[60%] group-hover:top-[30%] z-10 bg-[#C9FD74]">
    ...
    <div className="w-full flex justify-between p-4">
      <div className="flex flex-col">
        <h2 className='font-custom leading-1 text-4xl capitalize text-[#212121]'>{card.title}</h2>
        <h2 className='font-custom leading-1 text-4xl capitalize text-[#212121]'>{card.title2}</h2>
      </div>
      <h2 className='font-normal text-para text-[#212121]'>{card.id}</h2>
    </div>
    ...
    <div className="bg-slat-400 p-4 pt-0">
      {card?.subSevice?.map(({ title, logo }, index) => (
        <div key={index} className="w-full flex items-center justify-between py-2 border-b">
          <h2 className='font-normal text-para text-[#212121]'>{title}</h2>
